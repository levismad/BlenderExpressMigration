...
{
    public class C13Repository : BaseRepository<C13Model>
    {
        private const string SqlOrigem = "Levismad.ComandoSQLOrigem.C13";
        private const string SqlDestino = "Levismad.ComandoSQLDestino.C13";
        
        public C13Repository() : base(SqlOrigem, SqlDestino, 24, GetModels, GetParameters)
        {
        }

        public void EfetuarCarga()
        {
            EfetuarCargaDefault("C13", Finally, Math.Max(1, StringUtil.LastInsertC13));
        }

        private static int Finally(int index)
        {
            StringUtil.LastInsertC13 = index + 1;
            return 0;
        }
        private static C13Model GetModels(List<object> reader, int i)
        {
            #region Campos
            var o1 = reader[0];
            var o2 = reader[1];
            ...
            var model = new C13Model
            {
                ...
                Traidtsitconta = o15.ToString() == "" ? (long?) null : long.Parse(o15.ToString()),
        ...
        private static List<OracleParameter> GetParameters(List<C13Model> models)
        {
